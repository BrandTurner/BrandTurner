...
BrandTurner.Routers = BrandTurner.Routers || {};

(function () {
  'use strict';

    BrandTurner.Routers.Router = Backbone.Router.extend({
    controller: null
    , currentView: null
    , currentModal: null
    , history: []
    ...
    , initialize: function (options) {
      ...
      this.$app = $('<div class="brandturner"/>');
      this.$modal = $('<div />');
    ...
    , setCurrentModal: function (view) {
      ...
      var modal = new BrandTurner.Views.Modal({
     ...
     */
    , home: function () {
      var view = new BrandTurner.Views.Home({
    ...
    , error: function (name) {
      ...
      var view = new BrandTurner.Views.Error({
        model: data
        , controller: this.controller
      });
    ...
    , loading: function (name) {
      ...
      var view = new BrandTurner.Views.Loading({
        model: data
        , controller: this.controller
      });
      this.setCurrentModal(view);
